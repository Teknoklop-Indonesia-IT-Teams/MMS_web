...
import DeviceCardPLC from "./DeviceCardPLC";
import MaintenanceDashboardPLC from "./MaintenanceDashboardPLC";
import { useDashboardDataPLC } from "../../hooks/useLazyEquipmentPLC";
...
import PMDashboardPLC from "./PMDashboardPLC";
import CMDashboardPLC from "./CMDashboardPLC";

const DashboardPLC: React.FC = () => {
  ...
  const { loading, isDataLoaded, stats, equipmentPLC } = useDashboardDataPLC();
  ...
  useEffect(() => {
    console.log("DashboardPLC equipment count:", equipmentPLC.length);
  }, [equipmentPLC]);
  ...
  const equipmentByType = equipmentPLC.reduce(
    (acc, curr) => {
  ...
  useEffect(() => {
    console.log("EquipmentPLC:", equipmentPLC);
    console.log(
      "Jenis list:",
      equipmentPLC.map((e) => e.jenis),
    );
  }, [equipmentPLC]);
  ...
  const getMaintenanceStats = (data: EquipmentPLC[]) => {
    let selesai = 0;
    let urgent = 0;
    let warning = 0;
    let normal = 0;
  ...
  const filteredEquipment = useMemo(() => {
    if (selectedFilter === "all") {
      ...
      return equipmentPLC.filter((item) => item.jenis === selectedFilter);
    }
  }, [equipmentPLC, selectedFilter]);
  ...
  return (
    <div className="p-6">
      ...
      {/* Maintenance DashboardPLC */}
      <MaintenanceDashboardPLC equipment={filteredEquipment} />
          ...
          <div className="space-y-3 text-sm">
            {paginatedActivity.length > 0 ? (
              paginatedActivity.map((item, index) => {
                ...
                const getStatusInfo = (equipment: EquipmentPLC) => {
