...
const PlcDescriptionModal: React.FC<Props> = ({ equipment, onClose }) => {
    const Row = ({ label, value }: { label: string; value?: string }) => (
        <div>
            ...
            <p className="p-2 text-sm text-gray-900 rounded bg-gray-50 dark:bg-gray-700 dark:text-gray-100">{value || "-"}</p>
        </div>
    );

    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto dark:bg-gray-800">
                <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
                    <div>
                        <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100">Detail Informasi Alat PLC</h2>
                        <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                            {equipment.nama} — {equipment.jenis}
                        </p>
                    </div>
                    <button onClick={onClose} className="p-2 text-gray-400 rounded-lg hover:text-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700">
                        <X size={20} />
                    </button>
                </div>
                ...
                <div className="p-6">
                    <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <span className={`px-2 py-1 text-sm rounded inline-block ${equipment.status === "Garansi" ? "bg-green-100 text-green-800" :
                                        equipment.status === "Habis" ? "bg-red-100 text-red-800" :
                    ...
                    {equipment.i_alat && (
                        <div className="mt-6">
                            <label className="block mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">Gambar Alat</label>
