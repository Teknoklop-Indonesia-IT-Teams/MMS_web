...
function ImageThumbnail({ src, alt }: { src: string; alt?: string }) {
  ...
  const full = src.startsWith("http")
    ? src
    : `${import.meta.env.VITE_URL || "http://localhost:3001"}${src}`;
  return (
    <>
      <div
      ...
      {showLightbox && (
        <Lightbox src={full} alt={alt} onClose={() => setShowLightbox(false)} />
      )}
    </>
  );
}
      ...
      >
        <div className="flex items-center justify-between p-6 border-b dark:border-gray-700">
          <h2 className="text-xl font-semibold dark:text-gray-100">
            Detail Record Corrective PLC — {equipment.nama}
          </h2>
          <button
            onClick={onClose}
          ...
          <div className="grid grid-cols-1 gap-6 mb-8 lg:grid-cols-2">
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                [
                  "Instalasi",
                  equipment.instalasi ? equipment.instalasi.split("T")[0] : "-",
                ],
                [
                  "Garansi",
                  equipment.garansi ? equipment.garansi.split("T")[0] : "-",
                ],
              ].map(([label, value]) => (
                <div key={label}>
                ...
                <form onSubmit={handleSave} className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        value={formData.i_panel}
                        onChange={(e) =>
                          setFormData({ ...formData, i_panel: e.target.value })
                        }
                        className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-600 dark:border-gray-500 dark:text-white"
                    ...
                    <div>
                      <label className="block mb-1 text-sm font-medium text-gray-700 dark:text-gray-300">
                        Info Sensor (i_sensor)
                      </label>
                      <input
                        type="text"
                        value={formData.i_sensor}
                        onChange={(e) =>
                          setFormData({ ...formData, i_sensor: e.target.value })
                        }
                        className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-600 dark:border-gray-500 dark:text-white"
