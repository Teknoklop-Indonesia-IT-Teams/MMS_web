...
import { alatPlcService, recordPlcService, staffService } from "../../../services/api";
...
function Lightbox({ src, alt, onClose }: { src: string; alt?: string; onClose: () => void }) {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black bg-opacity-90" onClick={onClose}>
            <button className="absolute p-2 text-white bg-gray-700 rounded-full top-4 right-4 hover:bg-gray-600" onClick={onClose}>
                <X size={20} />
            </button>
    ...
    return (
        <>
            <div className="relative cursor-pointer w-14 h-14 group" onClick={() => setShowLightbox(true)}>
                <img src={fullSrc} alt={alt || "Record Image"}
...
export default function PlcPreventiveDetail({ equipment, onClose, onUpdate }: Props) {
    ...
    const fetchEquipmentStatus = useCallback(async () => {
        ...
        } catch (e) { console.error("Error fetching PLC equipment status:", e); }
    }, [equipment.id]);
    ...
    const toggleExpand = (id: number) => setExpandedRecordId((prev) => (prev === id ? null : id));
    ...
    function handleImageChange(e: React.ChangeEvent<HTMLInputElement>) {
        ...
        const validFiles = files.filter((f) => {
            ...
            if (f.size > 5 * 1024 * 1024) { alert(`${f.name}: Maks 5MB.`); return false; }
            return true;
        });
                ...
                <div className="p-6">
                    <div className="grid grid-cols-1 gap-6 mb-8 lg:grid-cols-2">
                        <div className="grid grid-cols-2 gap-4">
                            {[
                                ...
                                ["Instalasi", formatDate(equipment.instalasi)], ["Garansi", formatDate(equipment.garansi)],
                            ].map(([label, value]) => (
                                <div key={label}>
                    ...
                    <div className="pt-6 border-t dark:border-gray-700">
                        ...
                        <div className="space-y-4">
                            ...
                            {isAdmin && (
                                <MaintenanceActions equipment={equipmentWithStatus} onUpdate={() => {
                                    void fetchEquipmentStatus(); void fetchRecords(); onUpdate?.();
                                }} />
                            )}
                        </div>
                    </div>
                                ...
                                <form onSubmit={handleSaveRecord} className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                                        {[
                                            ...
                                            { label: "Spareparts", key: "tambahan", required: false, placeholder: "Sparepart yang diganti (isi - jika tidak ada)..." },
                                        ...
                                        <div>
                                            ...
                                            <label htmlFor="plc-record-image"
                                                ...
                                                <input id="plc-record-image" type="file"
                        ...
                        <div className="overflow-hidden bg-white border rounded-lg dark:bg-gray-800 dark:border-gray-700">
                            <table className="w-full">
                                ...
                                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                                    {records.length === 0 && (
                                        <tr><td colSpan={7} className="px-4 py-8 text-sm text-center text-gray-400">Belum ada record preventive PLC.</td></tr>
                                    )}
                                    {records.map((record) => (
                                                ...
                                                <td className="px-4 py-4">
                                                    {record.i_alat && record.i_alat.length > 0 ? (
                                                        <div className="relative inline-block cursor-pointer"
                                                            ...
                                                            title={record.i_alat.length > 1 ? `Klik untuk lihat semua ${record.i_alat.length} gambar` : "Klik untuk detail"}>
                                                            ...
                                                            {record.i_alat && record.i_alat.length > 0 && (
                                                                <div className="md:col-span-3">
                                                                    ...
                                                                    <div className="flex flex-wrap gap-2">
                                                                        {record.i_alat.map((src: string, idx: number) => (
