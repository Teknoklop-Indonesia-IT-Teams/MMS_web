...
export default function EquipmentDetail({ equipment, onClose, onUpdate }: EquipmentDetailProps) {
  ...
  const fetchEquipmentStatus = useCallback(async () => {
    try { const r = await alatService.getWithMaintenanceStatus(equipment.id); setEquipmentWithStatus(r.data); }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50">
      <div className="w-full max-w-6xl max-h-screen overflow-y-auto bg-white rounded-lg shadow-xl dark:bg-gray-800">
        <div className="flex items-center justify-between p-6 border-b">
          <h2 className="text-xl font-semibold dark:text-gray-100">Detail Record Maintenance {equipment.nama}</h2>
        ...
        <div className="p-6">
          ...
          <div className="pt-6 border-t">
            ...
            <div className="space-y-4">
              ...
              {isAdmin && (
                <MaintenanceActions equipment={equipmentWithStatus} onUpdate={() => { void fetchEquipmentStatus(); void fetchRecords(); onUpdate?.(); }} />
              )}
            </div>
          </div>
                  ...
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                    ...
                    <div>
                      ...
                      <label htmlFor="record-image-upload"
                        ...
                        <input id="record-image-upload" type="file" accept="image/jpeg,image/png,image/gif,image/webp" multiple className="hidden" onChange={handleImageChange} />
                      </label>
                    </div>
                  </div>
            ...
            <div className="overflow-hidden bg-white border rounded-lg dark:bg-gray-800 dark:border-gray-700">
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                  {records.length === 0 && (
                    <tr><td colSpan={7} className="px-4 py-8 text-sm text-center text-gray-400 dark:text-gray-500">Belum ada record maintenance.</td></tr>
                  )}
                  {records.map((record) => (
