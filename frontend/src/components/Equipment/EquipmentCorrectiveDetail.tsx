...
export default function EquipmentCorrectiveDetail({
  ...
  // ✅ Ganti dengan recordCorrectiveService
  const fetchRecords = useCallback(async () => {
    try {
      ...
      console.error("Error fetching corrective records:", error);
    }
  }, [equipment.id]);
  ...
  async function handleSaveRecord(e: React.FormEvent) {
    ...
    try {
      ...
      const response = await recordCorrectiveService.create(newRecord);
  ...
  async function handleDeleteRecord(recordId: number) {
    if (window.confirm("Apakah Anda yakin ingin menghapus record ini?")) {
      try {
        ...
        await fetchRecords();
        showSuccess("Corrective record berhasil dihapus");
      } catch (error) {
        console.error("Error deleting corrective record:", error);
        alert("Gagal menghapus corrective record. Silakan coba lagi.");
      }
    }
  }
